...
class RelaySMSMailPNBAAdapter(PNBAProtocolInterface):
    ...
    def _reserve_random_alias_prefix(self, store: RandomAliasStore) -> str:
        max_attempts = 10
        for _ in range(max_attempts):
            ...
            if store.register(alias_prefix):
                return alias_prefix
    ...
    def _create_random_alias(self, base_path: Optional[str] = None) -> AliasResponse:
        with RandomAliasStore(self.credentials, base_path) as store:
            alias_prefix = self._reserve_random_alias_prefix(store)
            timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S (%Z)")
            ...
            try:
                ...
                store.remove(alias_prefix)
                logger.error(
                    "Failed to create random alias '%s': %s", alias_prefix, exc
                )
                raise
    ...
    def send_message(self, phone_number: Optional[str] = None, **kwargs) -> bool:
        ...
        if phone_number:
            ...
            alias = self._create_random_alias(base_path)
